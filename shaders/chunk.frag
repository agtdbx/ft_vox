...
vec4    getColor()
{
    ...
    else if (fragNormal.y < -0.99) // Down
    {
        ...
        color = texture(sampleCubeTextures, vec3(texCoord, 36 + fragCubeType)) * 0.2;
    }
    else if (fragNormal.x > 0.99) // Right
    {
        texCoord = vec2(-fragPosition.z, -fragPosition.y);
        color = texture(sampleCubeTextures, vec3(texCoord, 18 + fragCubeType)) * 0.4;
    }
    else if (fragNormal.x < -0.99) // Left
    {
        texCoord =  vec2(fragPosition.z, -fragPosition.y);
        color = texture(sampleCubeTextures, vec3(texCoord, 18 + fragCubeType)) * 0.6;
    }
    else if (fragNormal.z > 0.99) // Front
    {
        texCoord = vec2(fragPosition.x, -fragPosition.y);
        color = texture(sampleCubeTextures, vec3(texCoord, 18 + fragCubeType)) * 0.9;
    }
    else if (fragNormal.z < -0.99) // Back
    {
        texCoord = vec2(-fragPosition.x, -fragPosition.y);
        color = texture(sampleCubeTextures, vec3(texCoord, 18 + fragCubeType)) * 0.3;
    }
